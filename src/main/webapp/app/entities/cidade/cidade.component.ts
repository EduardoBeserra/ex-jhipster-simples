...
export class CidadeComponent implements OnInit, OnDestroy {
    ...
    registerChangeInCidades() {
        this.eventSubscriber = this.eventManager.subscribe('cidadeListModification', response => this.reset());
    }
    ...
    private paginateCidades(data: ICidade[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.cidades.push(data[i]);
        }
    }
